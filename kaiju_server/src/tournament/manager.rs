...
impl TournamentManager {
            ...
            match state.as_str() {
                "Registration" => {
                    ...
                    if now >= start_time {
                        ...
                        if real_players == 0 {
                            tracing::info!(
                                "No real players in tournament {}, extending registration",
                                id
                            );
    ...
    async fn start_tournament(&self, tournament_id: &str) -> Result<(), anyhow::Error> {
        ...
        let participants: Vec<(String, bool)> = sqlx::query_as::<_, (String, bool)>(
            "SELECT kaiju_id, is_bot FROM tournament_participants WHERE tournament_id = ?",
        )
        .bind(tournament_id)
        .fetch_all(&self.pool)
        .await?;
        ...
        if count < target {
            ...
            self.add_bots(tournament_id, target - count, &mut participant_ids)
                .await?;
        }
    ...
    async fn process_round(&self, tournament_id: &str, round: i32) -> Result<(), anyhow::Error> {
        ...
        for m in matches {
            ...
            let a_name: String = sqlx::query_scalar("SELECT name FROM kaiju WHERE id = ?")
                .bind(&a_id)
                .fetch_one(&self.pool)
                .await?;
            let b_name: String = sqlx::query_scalar("SELECT name FROM kaiju WHERE id = ?")
                .bind(&b_id)
                .fetch_one(&self.pool)
                .await?;
    ...
    async fn fetch_stats(&self, kaiju_id: &str) -> Result<KaijuStats, anyhow::Error> {
        let json: sqlx::types::Json<KaijuStats> =
            sqlx::query_scalar("SELECT base_stats FROM kaiju WHERE id = ?")
                .bind(kaiju_id)
                .fetch_one(&self.pool)
                .await?;
        Ok(json.0)
    ...
    async fn advance_round(
        ...
        if winners.len() == 1 {
            ...
            sqlx::query(
                "UPDATE tournaments SET state = 'Finished', winner_kaiju_id = ? WHERE id = ?",
            )
            .bind(winner)
            .bind(tournament_id)
            .execute(&self.pool)
