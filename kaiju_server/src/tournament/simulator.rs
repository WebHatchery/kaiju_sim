...
pub fn simulate_battle(a_name: &str, a: &KaijuStats, b_name: &str, b: &KaijuStats) -> BattleResult {
    ...
    // Simple calc for now
    let score_a = (a.hp as f32) + (a.attack as f32 * 2.0) + (a.defense as f32 * 1.5) + (a.speed as f32);
    let score_b = (b.hp as f32) + (b.attack as f32 * 2.0) + (b.defense as f32 * 1.5) + (b.speed as f32);
    ...
    log.push(format!("{} Power: {:.0} (Luck: {:.2})", a_name, final_a, variance_a));
    log.push(format!("{} Power: {:.0} (Luck: {:.2})", b_name, final_b, variance_b));
