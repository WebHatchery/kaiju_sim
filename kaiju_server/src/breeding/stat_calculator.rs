...
use super::{BreedingConfig, HybridElement, StatBonus, StatRoll};
use rand::Rng;
...
impl StatCalculator {
    ...
    pub fn calculate_offspring_stats<R: Rng>(
        ...
        let attack = self.calc_stat(
            ...
            c.floors.attack,
            c.base_caps.attack,
            c.hard_ceilings.attack,
            stabilizer,
            rng,
            &mut rolls,
        );
        let defense = self.calc_stat(
            "defense",
            parent_a.defense,
            parent_b.defense,
            generation,
            c.floors.defense,
            c.base_caps.defense,
            c.hard_ceilings.defense,
            stabilizer,
            rng,
            &mut rolls,
        );
        let speed = self.calc_stat(
            "speed",
            parent_a.speed,
            parent_b.speed,
            generation,
            c.floors.speed,
            c.base_caps.speed,
            c.hard_ceilings.speed,
            stabilizer,
            rng,
            &mut rolls,
        );
        let energy = self.calc_stat(
            "energy",
            parent_a.energy,
            parent_b.energy,
            generation,
            c.floors.energy,
            c.base_caps.energy,
            c.hard_ceilings.energy,
            stabilizer,
            rng,
            &mut rolls,
        );

        let mut stats = KaijuStats {
            hp,
            attack,
            defense,
            speed,
    ...
    fn calc_stat<R: Rng>(
        ...
        stabilizer: bool,
        rng: &mut R,
        rolls: &mut HashMap<String, StatRoll>,
    ) -> i32 {
        let base = (a + b) as f32 / 2.0;
        let g = gen as f32;
        let dim = 1.0 / (1.0 + (g / 20.0));
        let gen_mult = (1.0 + g * self.config.stats.generation_power_creep) * dim;
        let variance = if stabilizer {
            rng.gen_range(1.0..=self.config.stats.variance_max)
        } else {
            rng.gen_range(self.config.stats.variance_min..=self.config.stats.variance_max)
        };
        let raw = base * gen_mult * variance;
        let mut caps = Vec::new();
        let floored = if raw < floor as f32 {
            caps.push(format!("floor:{}", floor));
            floor as f32
        } else {
            raw
        };
        let soft_cap =
            (base_cap as f32 * (1.0 + gen as f32 * self.config.stats.soft_ceiling_scaling)) as i32;
        let soft_capped = if floored > soft_cap as f32 {
            caps.push(format!("soft:{}", soft_cap));
            soft_cap as f32
        } else {
            floored
        };
        let final_val = if soft_capped > hard_cap as f32 {
            caps.push(format!("hard:{}", hard_cap));
            hard_cap
        } else {
            soft_capped as i32
        };
        rolls.insert(
            ...
            StatRoll {
                roll: variance,
                formula: format!("({} + {}) / 2 * {:.3} * {:.3}", a, b, gen_mult, variance),
                base_value: base,
                gen_multiplier: gen_mult,
                variance,
                result: final_val,
                caps_applied: caps,
            },
    ...
    fn apply_hybrid(
        ...
        match &h.primary_stat_bonus {
            StatBonus::Speed(b) => {
                stats.speed += b;
                if let Some(r) = rolls.get_mut("speed") {
                    r.result = stats.speed;
                }
            }
            StatBonus::Attack(b) => {
                stats.attack += b;
                if let Some(r) = rolls.get_mut("attack") {
                    r.result = stats.attack;
                }
            }
            StatBonus::Defense(b) => {
                stats.defense += b;
                if let Some(r) = rolls.get_mut("defense") {
                    r.result = stats.defense;
                }
            }
            StatBonus::Hp(b) => {
                stats.hp += b;
                if let Some(r) = rolls.get_mut("hp") {
                    r.result = stats.hp;
                }
            }
            StatBonus::Evasion(_) => {}
            StatBonus::Multiple(bonuses) => {
                for (n, b) in bonuses {
                    match n.as_str() {
                        "speed" => {
                            stats.speed += b;
