...
pub fn draw_kaiju_card(
    ...
    let border_width = if effective_state == CardState::Selected { 2.0 } else { 1.0 };
    ...
    if !drawn {
        ...
        draw_rectangle(x + 10.0, y + 10.0, portrait_size, portrait_size, Color::new(0.15, 0.15, 0.15, 1.0));
        draw_text_centered("?", x + 10.0 + portrait_size/2.0, y + 10.0 + portrait_size/2.0, FONT_LARGE, GRAY);
    }
    
    // Generation badge (Top Left overlay)
    draw_rectangle(x + 10.0, y + 10.0, 50.0, 20.0, Color::new(0.0, 0.0, 0.0, 0.7));
    draw_text(&format!("Gen {}", kaiju.generation), x + 15.0, y + 24.0, FONT_TINY, WHITE);
    
    // Trophy Badge (Top Right)
    if kaiju.tournaments_won > 0 {
        draw_rectangle(x + 10.0 + portrait_size - 60.0, y + 10.0, 60.0, 20.0, GOLD);
        draw_text(&format!("Wins: {}", kaiju.tournaments_won), x + 10.0 + portrait_size - 55.0, y + 24.0, FONT_TINY, BLACK);
    }
    ...
    draw_stat_row(x + 10.0, stats_y + stat_spacing, "ATK", kaiju.stats.attack, dark::ATK_COLOR);
    draw_stat_row(x + 10.0, stats_y + stat_spacing * 2.0, "DEF", kaiju.stats.defense, dark::DEF_COLOR);
    draw_stat_row(x + 10.0, stats_y + stat_spacing * 3.0, "SPD", kaiju.stats.speed, dark::SPD_COLOR);
    
    // Status overlay for dead kaiju
    if !kaiju.alive {
        draw_rectangle(x, y, CARD_WIDTH, CARD_HEIGHT, Color::new(0.0, 0.0, 0.0, 0.6));
    ...
    // Select button
    let select_bg = if select_hovered { dark::ACCENT } else { dark::BUTTON_BG };
    draw_rectangle(x + 5.0, btn_y, btn_w, 28.0, select_bg);
    draw_text_centered("Select", x + 5.0 + btn_w / 2.0, btn_y + 19.0, FONT_SMALL, WHITE);
    
    // View button
    let view_bg = if view_hovered { dark::ACCENT } else { dark::PANEL };
    draw_rectangle(x + btn_w + 15.0, btn_y, btn_w, 28.0, view_bg);
    draw_text_centered("View", x + btn_w + 15.0 + btn_w / 2.0, btn_y + 19.0, FONT_SMALL, WHITE);
