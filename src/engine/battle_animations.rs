...
impl AnimationState {
    pub fn is_complete(&self) -> bool {
        matches!(self, AnimationState::Idle | AnimationState::Victory | AnimationState::Defeat)
    }
}
        ...
        match self.state.clone() {
            ...
            AnimationState::WindUp { progress } => {
                ...
                if new_progress >= 1.0 {
                    ...
                    self.state = AnimationState::WindUp { progress: new_progress };
                    // Pull back slightly
                    self.attacker_pos = self.attacker_base - self.attack_offset.normalize() * 20.0 * new_progress;
                }
            }
            ...
            AnimationState::Strike { progress } => {
                ...
                if new_progress >= 1.0 {
                    ...
                    self.state = AnimationState::Strike { progress: new_progress };
                    // Lunge forward
            ...
            AnimationState::Impact { progress } => {
                ...
                if new_progress >= 1.0 {
                    ...
                    self.state = AnimationState::Impact { progress: new_progress };
                    // Screen shake
                }
            }
            ...
            AnimationState::Recoil { progress } => {
                ...
                if new_progress >= 1.0 {
                    ...
                    self.state = AnimationState::Recoil { progress: new_progress };
                    // Defender recoils
                    let t = ease_out_quad(new_progress);
            ...
            AnimationState::Return { progress } => {
                ...
                if new_progress >= 1.0 {
                    ...
                    self.state = AnimationState::Return { progress: new_progress };
                    // Return to base
                    let t = ease_out_quad(new_progress);
...
impl DamageNumber {
    ...
    pub fn draw(&self) {
        ...
        // Shadow
        draw_text(&text, self.position.x + 2.0, self.position.y + 2.0, font_size, Color::new(0.0, 0.0, 0.0, alpha * 0.5));
...
impl DamageNumberManager {
    ...
    pub fn spawn(&mut self, value: i32, position: Vec2, is_critical: bool, is_healing: bool) {
        self.numbers.push(DamageNumber::new(value, position, is_critical, is_healing));
    }
