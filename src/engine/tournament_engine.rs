...
use crate::data::tournament::{
    Bracket, Match, MatchResult, Round, RoundStatus, Tournament, TournamentError,
    TournamentStatus,
};
use crate::data::Kaiju;
use crate::engine::bracket_generator::{BracketGenerator, SingleEliminationGenerator, SwissGenerator, select_environment};
...
impl TournamentEngine {
    ...
    pub fn start_tournament(&self, tournament: &mut Tournament) -> Result<(), TournamentError> {
        ...
        let bracket = match &tournament.bracket_system {
            ...
            crate::data::tournament::BracketSystem::Swiss { rounds } => {
                SwissGenerator { total_rounds: *rounds }
                    .generate(tournament.id, tournament.participants.clone(), environment)
                    .map_err(|_| TournamentError::InsufficientParticipants)?
            }
    ...
    pub fn execute_match(
        ...
        match_obj.result = Some(MatchResult {
            ...
            final_hp: (
                if winner_is_a { battle_result.hp_remaining } else { 0 },
                if winner_is_a { 0 } else { battle_result.hp_remaining },
            ),
            was_upset,
            upset_probability,
        });
    ...
    pub fn advance_round(
        ...
        let bracket = tournament.bracket.as_mut().ok_or(TournamentError::RoundNotComplete)?;
        ...
        if winners.len() <= 1 {
            if let Some(winner_id) = winners.first() {
                tournament.status = TournamentStatus::Completed { winner_id: *winner_id };
            }
            return Ok(Vec::new());
        }
        ...
        if let Some(round) = bracket.rounds.iter_mut().find(|r| r.number == current_round) {
            round.status = RoundStatus::Completed;
        }
        
        tournament.current_round = next_round;
        tournament.status = TournamentStatus::InProgress { current_round: next_round };
...
pub fn calculate_xp_reward(
    ...
    let multiplier = match tournament_type {
        ...
        crate::data::tournament::TournamentType::SpecialEvent { xp_multiplier, .. } => *xp_multiplier,
        _ => 1.0,
    };
