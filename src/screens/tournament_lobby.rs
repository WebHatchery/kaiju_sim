...
thread_local! {
    ...
    static FETCH_ERROR: RefCell<Option<String>> = const { RefCell::new(None) };
}

pub fn reset_state() {
    INITIALIZED.with(|initialized| initialized.set(false));
    FETCH_ERROR.with(|error| *error.borrow_mut() = None);
}

fn set_fetch_error(error: Option<String>) {
    FETCH_ERROR.with(|stored| *stored.borrow_mut() = error);
}

fn fetch_error() -> Option<String> {
    FETCH_ERROR.with(|stored| stored.borrow().clone())
}

pub fn draw_tournament_lobby(state: &mut GameState) -> Option<UiAction> {
    // Fetch once on first entry to this screen
    if !INITIALIZED.with(|initialized| initialized.get()) {
        match server_bridge::get_current_tournament() {
            Ok(info) => {
                set_fetch_error(None);
                state.tournament_status = Some(info);
                INITIALIZED.with(|initialized| initialized.set(true)); // Only set after successful fetch
            }
            Err(e) => {
                set_fetch_error(Some(e));
                INITIALIZED.with(|initialized| initialized.set(true)); // Set anyway to avoid spam
            }
        }
    }
    ...
    if let Some(info) = &state.tournament_status {
        ...
        draw_text(
            &format!("Round: {}", info.current_round),
            200.0,
            content_y,
            FONT_MEDIUM,
            WHITE,
        ...
        draw_text(
            &format!("Participants: {}", info.participants_count),
            400.0,
            content_y,
            FONT_MEDIUM,
            WHITE,
        ...
        match info.state.as_str() {
            "Registration" => {
                draw_text(
                    &format!("Next Round Starts: {}", info.start_time),
                    20.0,
                    sub_y,
                    FONT_NORMAL,
                    YELLOW,
                ...
                for k in valid_kaiju {
                    ...
                    draw_text(
                        &format!("Stats: {}", k.stats.power_level()),
                        btn_rect.x + 200.0,
                        btn_rect.y + 30.0,
                        FONT_SMALL,
                        GRAY,
                    ...
                    // Enroll Button
                    let enroll_success = draw_centered_btn(
                        "Enroll",
                        btn_rect.x + 300.0,
                        btn_rect.y + 10.0,
                        80.0,
                    ...
                    if enroll_success {
                        if let Ok(msg) = server_bridge::enroll_in_tournament(
                            Uuid::parse_str(&state.player.player_id).unwrap_or_default(),
                            k.id,
                        ) {
                            ...
                            state.notify(
                                ...
                                crate::state::game_state::NotificationType::Error,
                            );
                        }
                    }
            ...
            "Running" | "Finished" => {
                ...
                draw_text(
                    &format!("Current Brackets (Round {})", current_r),
                    20.0,
                    m_y,
                    FONT_MEDIUM,
                    WHITE,
                ...
                for m in &info.matches {
                    ...
                    let color_a = if winner == m.kaiju_a_id.as_deref() && winner.is_some() {
                        GREEN
                    } else {
                        WHITE
                    };
                    let color_b = if winner == m.kaiju_b_id.as_deref() && winner.is_some() {
                        GREEN
                    } else {
                        WHITE
                    };
        ...
        draw_text(
            "Connecting to Tournament Network...",
            20.0,
            content_y + 20.0,
            FONT_MEDIUM,
            GRAY,
